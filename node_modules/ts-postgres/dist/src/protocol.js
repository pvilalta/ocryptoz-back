...
function getMessageSize(segment, value) {
    switch (segment) {
        case 0 /* Buffer */: {
...
function readRowData(buffer, rowDataOffset, rowDescription, encoding, types) {
...
class Writer {
    ...
    bind(name, portal, format = types_1.DataFormat.Binary, values = [], types = []) {
        ...
        const addBinaryValue = (value, dataType) => {
            ...
            switch (dataType) {
                ...
                case types_1.DataType.Int8: {
                    size = add(6 /* Int64BE */, BigInt(value));
                    break;
                }
                case types_1.DataType.Point:
                    {
    ...
    ;
    password(text) {
        this.sendMessage(112 /* Password */, [makeBufferSegment(text, this.encoding, true)]);
    }
    send() {
    ...
    startup(user, database, extraFloatDigits) {
        const data = [
            'user',
            user,
            'database',
    ...
    sendMessage(code, segments) {
        const size = this.getMessageSize(code, segments);
        const buffer = Buffer.allocUnsafe(size);
        this.write(code, segments, buffer, size);
        this.stream.write(buffer);
    }
    write(code, segments, buffer, size) {
        let offset = 0;
        if (code)
            buffer[offset++] = code;
